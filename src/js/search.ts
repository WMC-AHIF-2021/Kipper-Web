...
export async function search(): Promise<void> {
  const overlayDark = <HTMLDivElement>(
    document.getElementById("search-background-overlay")
  );
  const overlaySearch = <HTMLDivElement>(
    document.getElementById("search-result-overlay")
  );
  ...
  const searchBarRect: DOMRect =
    searchBar.parentElement.getBoundingClientRect();
